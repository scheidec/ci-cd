...
- Apps, teams, and infrastructure grow in complexity
    - results in a complicated software development process
...
- Continuous Deployment
    - the final step
    - automatic deployment of each build that passes the full test cycle

---

## CD: Staging vs Production

- Makes the most sense to set up in the blog/shiny app projects
...
- Once tests pass and changes are approved in the staging environment, can be automatically pushed to production
...
- `when: manual`
    - exposes a play button in GitLab's UI and the `deploy_prod` job will only be triggered if and when we click that button
...
- __Pipelines -> Environments__ page
    ...
    - clicking on the environmnent will show the full history
